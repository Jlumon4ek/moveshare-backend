...
type TruckRepository interface {
	...
	DeleteTruckPhoto(ctx context.Context, userID, photoID int64) error
	GetTruckPhotoCount(ctx context.Context, truckID int64) (int, error)
}
...
// GetUserTrucks fetches all trucks owned by the given userID
...
// GetTruckByID fetches a truck by ID, ensuring it belongs to the user
...
// UpdateTruck updates a truck in the database
...
// DeleteTruck deletes a truck and all its photos
func (r *truckRepository) DeleteTruck(ctx context.Context, userID, truckID int64) error {
	// Note: truck_photos will be automatically deleted due to CASCADE constraint
...
func (r *truckRepository) GetTruckPhotos(ctx context.Context, userID, truckID int64) ([]TruckPhoto, error) {
	...
	_, err := r.GetTruckByID(ctx, userID, truckID)
	if err != nil {
		return nil, err
	}
...
// DeleteTruckPhoto deletes a photo
func (r *truckRepository) DeleteTruckPhoto(ctx context.Context, userID, photoID int64) error {
	query := `DELETE FROM truck_photos WHERE id = $1 AND user_id = $2`
	_, err := r.db.Exec(ctx, query, photoID, userID)
	return err
}

// GetTruckPhotoCount returns the number of photos for a truck
func (r *truckRepository) GetTruckPhotoCount(ctx context.Context, truckID int64) (int, error) {
	query := `SELECT COUNT(*) FROM truck_photos WHERE truck_id = $1`
	var count int
	err := r.db.QueryRow(ctx, query, truckID).Scan(&count)
	return count, err
}