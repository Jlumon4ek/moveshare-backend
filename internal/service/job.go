...
func (s *JobService) GetAvailableJobs(userID int64, page, limit int) ([]models.AvailableJobDTO, int, error) {
	ctx := context.Background()
	offset := (page - 1) * limit
	jobs, err := s.jobRepo.GetAvailableJobs(ctx, userID, offset, limit)
	if err != nil {
		return nil, 0, err
	}

	total, err := s.jobRepo.GetCountAvailableJobs(ctx, userID)
	if err != nil {
		return nil, 0, err
	}
